...
import { EntityArrayResponseType, OrderLineService } from '../service/order-line.service';
...
export class OrderLineComponent implements OnInit {
  ...
  trackId = (item: IOrderLine): number => this.orderLineService.getOrderLineIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.orderLines = this.refineData(dataFromBody);
  }

  protected refineData(data: IOrderLine[]): IOrderLine[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IOrderLine[] | null): IOrderLine[] {
    return data ?? [];
  }
